...
def _filter_error_rows(errors: dict,
    ...
    _logger.debug(f'type(validated_input) = {type(validated_input)}')
    _logger.debug(f'len(validated_input) = {len(validated_input)}')
    indexes = list(errors.keys())
...
def validate_inputs(input_data):
    ...
    _logger.debug(f'ON RECEPTION: type(input_data) should be str and it is ={type(input_data)}')

    schema = HouseDataRequestSchema(many=True)
    input_data = pd.read_json(input_data)      # input_data is in json format. To be converted to pd.DataFrame
    # _logger.error(f'VAL VAL input_data = {input_data}')
    # convert syntax error field names (beginning with numbers)
    for dict in input_data:
        for key, value in SYNTAX_ERROR_FIELD_MAP.items():
            if dict == key :
                input_data[value] = input_data[key]
                del input_data[key]

    # _logger.error(f'AFTER 1 : input_data ={input_data}')
    input_data = json.loads(input_data.to_json(orient='records')) # input_data is a pd.DataFrame. To be converted to json then to dict
    errors = None
    try:
        # schema.load() requires a dict to process the validation
        schema.load(input_data)
    except ValidationError as exc:
        errors = exc.messages
    # _logger.error(f'AFTER 2: input_data ={input_data}')

    input_data = pd.read_json(json.dumps(input_data)) # input_data is dict. To be converted to json then to pd.DataFrame
